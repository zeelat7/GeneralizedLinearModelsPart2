...
# <- read.csv("logit_insurance_test.csv")
#call libraries
...
# cor_wine <- subset(wine, select = c(AcidIndex_IMP, Alcohol, Chlorides, CitricAcid, Density, FixedAcidity,
#                                     FreeSulfurDioxide, LabelAppeal, ResidualSugar, STARS, Sulphates, 
#                                     TotalSulfurDioxide, VolatileAcidity, pH), na.rm = TRUE)
# c <- cor(cor_wine)
...
# md.pattern(wine)
# 
# wine_temp <- subset(wine, select = c(INDEX, TARGET, AcidIndex, Alcohol, Chlorides, CitricAcid, Density, FixedAcidity, 
#                                      FreeSulfurDioxide, LabelAppeal, ResidualSugar, STARS, Sulphates, 
#                                      TotalSulfurDioxide, VolatileAcidity, pH))
# 

# summary(wine)
# wine_impute <- mice(wine_temp,m=5,maxit=50,meth='pmm',seed=500)
# summary(wine_impute)
...
# wine_run <-as.data.frame(wine_impute) 
# 
# reg2 <- regsubsets(TARGET ~ AcidIndex + Alcohol + Chlorides + CitricAcid + Density + FixedAcidity +
#                      FreeSulfurDioxide + LabelAppeal + ResidualSugar + STARS + Sulphates + TotalSulfurDioxide +
#                     VolatileAcidity + pH, data = wine_impute, nvmax = 50)
# summary(reg2)
